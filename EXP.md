...
- 결정 사항:
  ...
  5. **스케줄러 = APScheduler in-process** (단일 broker 인스턴스 가정 — §11 A10 따름). 작업 종류 3종:
     - (a) `cleanup_starts_at` 도달 시 **cleanup 자동 시작**(사용 시간 만료 — 모달 아님). T15 종료 임박 알림(10분/1분 전)은 별도 토스트로 안내하되 종료 모달은 띄우지 않는다(2026-05-28 정정: 이전엔 "모달 push + T14 0014 이중 안전망"이었으나 10분 전은 자동 cleanup이 맞음 — 모달은 명시 종료 P3 ①②에서만).
...
- (2026-05-27 정책 변경 후속 — §11 A14, 패치 시리즈 0012~ 예정) — 자동 페어링·자동 스트림 위에 종료 모달과 명시/암묵 종료 신호 라우트를 더한다. 패치 번호 0012부터 순차, 빌드 toolchain (Qt 6.7 `msvc2019_64` + MSVC v143/v144) 동일. **Stage A 머지 — 2026-05-28 (패치 0012 단일 커밋 `2b298554`: 종료 다이얼로그 + 단축키 통합)** (0014는 정책 재확인 후 철회 — 아래). 아래 0012/0013은 논리적 단계 설명이며 실제 패치 파일은 0012 하나. Stage B(0015/0016)는 T09 본구현·T11 후속 ②③ 의존이라 후속:
  - ✅ **0012 종료 확인 다이얼로그 컴포넌트 (2026-05-28 완료)** — 초안은 OverlayManager OSD 텍스트였으나 e2e에서 한글 미지원(`ModeSeven.ttf` 라틴 전용 → 두부글자) + 모달감 부족 발견 → **네이티브 다이얼로그로 전환**(사용자 선택). `Session::scShowTerminateConfirmDialog()`가 Windows에선 **`TaskDialogIndirect`**(Win10/11 룩 — 경고 아이콘 + 큰 제목 + 본문 + [종료]/[취소], 부모=스트림 창 `SDL_GetWindowWMInfo` HWND + `TDF_POSITION_RELATIVE_TO_WINDOW` 중앙), 그 외/실패 시 `SDL_ShowMessageBox` 폴백(file-scope `scTerminateConfirmViaTaskDialog`/`scTerminateConfirmViaSdl` helper). 한글은 `QStringLiteral`(소스 UTF-8 + `/utf-8`) → UTF-16 wstring. 안전 기본값 = 취소(Enter·Esc, `nDefaultButton=IDNO` + `TDF_ALLOW_DIALOG_CANCELLATION`). `comctl32.lib`(`#pragma comment`), Common-Controls v6 manifest는 빌드에 이미 포함. 카피 §11 A14 P5 압축본. `m_ScTerminateDialogActive` 재진입 가드. **다이얼로그는 항상 execInternal SDL 루프 스레드에서만 표시**(네이티브 다이얼로그는 video subsystem 스레드 전제). 단축키 0013이 `scRequestTerminateConfirm()`로 호출, 호스트 셧다운 0016도 재사용 예정(0014는 철회 — 10분 전은 서버 자동 cleanup).
  - ✅ **0013 단축키 인터셉트 (2026-05-28 완료)** — `app/streaming/input/keyboard.cpp`의 `performSpecialKeyCombo(KeyComboQuit)`를 즉시 `SDL_QUIT` push 대신 `Session::get()->scRequestTerminateConfirm()`로 변경. 후자는 `SDL_PushEvent(SDL_USEREVENT, SDL_CODE_SC_TERMINATE_CONFIRM)`만 — execInternal 루프의 `SDL_USEREVENT` 디스패치가 받아 `scShowTerminateConfirmDialog()` 호출(스레드 안전). 다이얼로그가 자체 버튼/모달을 가지므로 OSD 방식의 Y/N 키 게이트는 제거. `// TODO(Stage B): reportExplicitTermination("shortcut")` 마커는 `scShowTerminateConfirmDialog` 종료 분기에.
  - ❌ **0014 `cleanup_starts_at` 자동 타이머 — 철회 (2026-05-28)** — 한때 "10분 전 클라이언트 자동 모달 타이머 + URL `expires-at` 파라미터"로 구현했으나, 정책 재확인 결과 **`cleanup_starts_at`(10분 전)은 모달이 아니라 자동 cleanup 진입**(서버 T09 책임, A14 P3 정정)임이 확정됨 → 클라이언트가 그 시점에 할 일이 없어 전면 롤백. 롤백 범위: `frontend/src/lib/moonlight.ts::buildMoonlightUrl`(expires-at 제거)·`useMoonlightConnect`·`moonlight.test.ts`, Moonlight `main.cpp`·`cli/commandlineparser.{h,cpp}`·`ComputerManager`·`NvComputer::pendingExpiresAt`·`Session`(`scScheduleCleanupModalTimer`/deadline 멤버) 모두 원복. 10분 전 종료 임박 안내는 T15(토스트, 모달 아님)가 담당. Stage A 실제 산출 = **패치 0012 단일**(종료 다이얼로그 + 단축키, 커밋 `2b298554`).
...
- 카테고리: 풀스택
- 의존성: T09, T13
- 정책 출처: §11 A14 — 10분 전 시점은 **`cleanup_starts_at`과 일치**(A14 P1)이며 그때 서버(T09)가 **자동 cleanup**을 시작한다(모달 아님 — A14 P3 정정 2026-05-28). T15는 그 시점에 **종료 임박 토스트 알림**을 띄운다(명시 종료 모달과 다름 — 모달은 단축키·호스트 셧다운에서만). Broker→Moonlight 채널은 T14 후속 0016과 공유. 1분 전 알림은 cleanup 진행 중임을 알리는 추가 토스트.
- 완료 조건
...
- **A14 (예약 cleanup / grace / 명시-암묵 종료 정책, 2026-05-27 결정)**: 예약 시간을 사용 구간 + cleanup 구간으로 분할하고, 종료 이벤트를 명시(explicit)/암묵(implicit)으로 분류한다. 이 정책의 **단일 출처(single source of truth)** — T09/T12/T11/T14 후속/T07/T17/T16 보정은 모두 본 항목을 참조하며 수치를 본문에 중복 정의하지 않는다.
  ...
  - **P3. 종료 이벤트 분류**. `cleanup_starts_at`(10분 전) 도달은 **시간 만료에 의한 자동 cleanup 진입** — 모달 없이 서버(T09)가 cleanup을 시작한다(2026-05-28 정정: 이전엔 "10분 전 자동 모달"로 적었으나 실제 의도는 자동 cleanup). 모달이 뜨는 **명시 종료 트리거는 2개**: ① 사용 중 `Ctrl+Alt+Shift+Q` — Moonlight 인터셉트, ② **원격 사용자가 스트리밍 화면 안에서** 호스트 "시작→전원→시스템 종료" 클릭 — T11 agent가 `WM_QUERYENDSESSION` 후킹 → 실제 셧다운 차단 + Broker 통해 Moonlight에 모달 push(PC를 끄는 대신 세션 종료로 전환, 강의실 PC 원격 종료 사고 방지). 두 경우 모두 같은 모달, "종료" 클릭 시점이 명시 종료(cleanup 즉시 시작 + 사용권 반납 P4), "취소"는 세션 유지. **Broker가 explicit 신호를 받지 않은 모든 disconnect는 암묵 종료**(기본값 안전 → grace). 분류 판정 주체 = Broker.
