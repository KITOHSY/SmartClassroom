...
| 0012 | `0012-T14-Stage-A-TaskDialog-Ctrl-Alt-Shift-Q.patch` | **T14 후속 Stage A** (2026-05-28) — 스트리밍 중 종료 확인 다이얼로그. `Session::scShowTerminateConfirmDialog()`가 Windows `TaskDialogIndirect`(한글 카피, 안전 기본값=취소) / `SDL_ShowMessageBox` 폴백. `Ctrl+Alt+Shift+Q`가 즉시 `SDL_QUIT` 대신 `scRequestTerminateConfirm()` → `SDL_USEREVENT`(`SDL_CODE_SC_TERMINATE_CONFIRM`)를 push해 execInternal 루프 스레드가 표시(스레드 안전). 명시 종료에서만 — 10분 전 자동 cleanup은 서버(T09). Broker 통지(0015)·셧다운 push(0016)는 T09 의존 Stage B | `app/streaming/session.{h,cpp}`, `app/streaming/input/keyboard.cpp` |
